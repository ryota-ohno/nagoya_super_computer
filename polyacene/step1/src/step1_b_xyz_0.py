...
from make_1_b_xyz import exec_gjf##計算した点のxyzfileを出す
...
def main_process(args):
    ...
    auto_csv_path = os.path.join(auto_dir,'step1.csv')
    if not os.path.exists(auto_csv_path):        
        df_E = pd.DataFrame(columns = ['a','b','theta','E_p2','machine_type','status','file_name'])##いじる
        ...
        for a in [a_init_prev]:
            for b in [b_init_prev]:
                ...
                if len(df_val_ab)==0:
                    ...
                    continue
                heri_list.append([a,b]);E_list.append(df_val_ab['E_p2'].values[0])
        if len(para_list) != 0:
