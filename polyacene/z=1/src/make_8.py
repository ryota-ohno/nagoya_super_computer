...
MONOMER_LIST = ["C5",'C6']
############################汎用関数###########################
def get_monomer_xyzR(monomer_name,Ta,Tb,Tc,x2,A3):
    ...
    xyz_array = np.matmul(xyz_array,Rod(-ex,x2).T)
...
def make_xyzfile(monomer_name,params_dict,structure_type):
    x1 = params_dict.get('x1',0.0);y1 = params_dict.get('x1',0.0); z1 = params_dict.get('z1',0.0)
    x2 = params_dict.get('x2',0.0); y2 = params_dict.get('y2',0.0); z2 = params_dict.get('z2',0.0)
    
    monomer_array_i = get_monomer_xyzR(monomer_name,0,0,0,0,0)
    
    monomer_array_p1 = get_monomer_xyzR(monomer_name,x1,y1,z1,0,0)##1,2がb方向
    monomer_array_p2 = get_monomer_xyzR(monomer_name,x2,y2,z2,0,0)##1,2がb方向
    monomer_array_t1 = get_monomer_xyzR(monomer_name,x1+x2,y1+y2,z1+z2,0,0)##1,2がb方向
    monomer_array_t2 = get_monomer_xyzR(monomer_name,x1-x2,y1-y2,z1-z2,0,0)##1,2がb方向
...
def make_gjf_xyz(auto_dir,monomer_name,params_dict,machine_type,structure_type):
    x1 = params_dict.get('x1',0.0); y1 = params_dict.get('y1',0.0); z1 = params_dict.get('z1',0.0)
    x2 = params_dict.get('x2',0.0); y2 = params_dict.get('y2',0.0); z2 = params_dict.get('z2',0.0)
