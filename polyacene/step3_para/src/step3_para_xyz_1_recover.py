...
def main_process(args):
    ...
    auto_csv_path = os.path.join(auto_dir,'step3_para.csv')
    if not os.path.exists(auto_csv_path):        
        df_E = pd.DataFrame(columns = ['cx','cy','cz','a','b','theta','Rt','Rp','E','E_p1','E_t1','E_t2','E_ip1','E_ip2','E_i02','E_ip3','E_ip4','E_it1','E_it2','E_it3','E_it4','machine_type','status','file_name'])##いじる
...
def listen(auto_dir,monomer_name,num_nodes,isTest):##args自体を引数に取るか中身をばらして取るかの違い
    ...
    for idx,row in zip(df_queue.index,df_queue.values):
        ...
        if len(E_list)!=12:
            ...
            len_queue-=1
            Ep1=float(E_list[0]);Et1=float(E_list[1]);Et2=float(E_list[2]);Eip1=float(E_list[3]);Eip2=float(E_list[4]);Eit1=float(E_list[5]);Eit2=float(E_list[6]);Eit3=float(E_list[7]);Eit4=float(E_list[8]);Ei02=float(E_list[9]);Eip3=float(E_list[10]);Eip4=float(E_list[11])##ここも計算する分子数に合わせて調整
            E = ((Eip1+Eip2+Ei02+Eip3+Eip4)/2+Eit1+Eit2+Eit3+Eit4+Ep1+Et2+Et2)*2##隣接20分子　2パターン
            #### TODO
            df_E.loc[idx, ['E_p1','E_t1','E_t2','E_ip1','E_ip2','E_it1','E_it2','E_it3','E_it4','E_i02','E_ip3','E_ip4','E','status']] = [Et1,Ep1,Et2,Eip1,Eip2,Eit1,Eit2,Eit3,Eit4,Ei02,Eip3,Eip4,E,'Done']
