...
from make_para_1 import exec_gjf##計算した点のxyzfileを出す
...
def main_process(args):
    ...
    auto_csv_path = os.path.join(auto_dir,'step3_para.csv')
    if not os.path.exists(auto_csv_path):        
        df_E = pd.DataFrame(columns = ['cx','cy','cz','a','b','theta','Rt','Rp','E','E_p1','E_t1','E_t2','E_i01','E_ip1','E_ip2','E_i02','E_ip3','E_ip4','E_it1','E_it2','E_it3','E_it4','machine_type','status','file_name'])##いじる
...
def listen(auto_dir,monomer_name,num_nodes,isTest):##args自体を引数に取るか中身をばらして取るかの違い
    ...
    fixed_param_keys = ['theta','cx','cy','cz']
    opt_param_keys = ['a','b','Rt','Rp']
    ...
    for idx,row in zip(df_queue.index,df_queue.values):
        ...
        if len(E_list)!=13:
            ...
            len_queue-=1
            Ep1=float(E_list[0]);Et1=float(E_list[1]);Et2=float(E_list[2]);Ei01=float(E_list[3]);Eip1=float(E_list[4]);Eip2=float(E_list[5]);Eit1=float(E_list[6]);Eit2=float(E_list[7]);Eit3=float(E_list[8]);Eit4=float(E_list[9]);Ei02=float(E_list[10]);Eip3=float(E_list[11]);Eip4=float(E_list[12])##ここも計算する分子数に合わせて調整
            E = ((Ei01+Eip1+Eip2+Ei02+Eip3+Eip4)/2+Eit1+Eit2+Eit3+Eit4+Ep1+Et1+Et2)*2##隣接20分子　2パターン
            #### TODO
            df_E.loc[idx, ['E_p1','E_t1','E_t2','E_i01','E_ip1','E_ip2','E_it1','E_it2','E_it3','E_it4','E_i02','E_ip3','E_ip4','E','status']] = [Et1,Ep1,Et2,Ei01,Eip1,Eip2,Eit1,Eit2,Eit3,Eit4,Ei02,Eip3,Eip4,E,'Done']
...
def get_params_dict(auto_dir, num_init,fixed_param_keys,opt_param_keys):
    ...
    for index in df_init_params_inprogress.index:##こちら側はinit_params内のある業に関する探索が終わった際の新しい行での探索を開始するもの ###ここを改良すればよさそう
        ...
        if isDone:
            opt_params_dict={'a':opt_params_matrix[0][0],'b':opt_params_matrix[0][1],'Rt':opt_params_matrix[0][2],'Rp':opt_params_matrix[0][3]}
            ...
            for i in range(len(opt_params_matrix)):
                opt_params_dict={'a':opt_params_matrix[i][0],'b':opt_params_matrix[i][1],'Rt':opt_params_matrix[i][2],'Rp':opt_params_matrix[i][3]}
...
def get_opt_params_dict(df_cur, init_params_dict,fixed_params_dict):
    ...
    a_init_prev = init_params_dict['a'];b_init_prev = init_params_dict['b'];Rt_init_prev = init_params_dict['Rt'];Rp_init_prev = init_params_dict['Rp']
            ...
            for b in [b_init_prev]:
                for Rt in [Rt_init_prev-0.1,Rt_init_prev,Rt_init_prev+0.1]:
                    for Rp in [Rp_init_prev-0.1,Rp_init_prev,Rp_init_prev+0.1]:
                        ...
                        a = np.round(a,1);b = np.round(b,1);Rt = np.round(Rt,1);Rp = np.round(Rp,1)
                        df_val_ab = df_val[
                            (df_val['cx']==cx)&(df_val['cy']==cy)&(df_val['cz']==cz)&(df_val['Rt']==Rt)&(df_val['Rp']==Rp)&(df_val['a']==a)&(df_val['b']==b)&
                            (df_val['status']=='Done')
                                     ]
                        if len(df_val_ab)==0:
                            para_list.append([a,b,Rt,Rp])
                            continue
                        heri_list.append([a,b,Rt,Rp]);E_list.append(df_val_ab['E'].values[0])
        if len(para_list) != 0:
            return False,para_list
        a_init,b_init,Rt_init,Rp_init = heri_list[np.argmin(np.array(E_list))]
        if a_init==a_init_prev and b_init==b_init_prev and Rt_init==Rt_init_prev and Rp_init==Rp_init_prev:
            return True,[[a_init,b_init,Rt_init,Rp_init]]
        else:
            a_init_prev=a_init;b_init_prev=b_init;Rt_init_prev=Rt_init;Rp_init_prev=Rp_init
