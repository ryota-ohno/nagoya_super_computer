...
def main_process(args):
    ...
    if not os.path.exists(auto_csv_path):        
        df_E = pd.DataFrame(columns = ['theta','a','b','za','zb','E','E1','E2','E3','E4','status'])##いじる
    ...
    if not os.path.exists(auto_csv_path1):        
        df_E_1 = pd.DataFrame(columns = ['theta','a','za','E1','machine_type','status','file_name'])##いじる
    ...
    if not os.path.exists(auto_csv_path2):        
        df_E_2 = pd.DataFrame(columns = ['theta','b','zb','E2','machine_type','status','file_name'])##いじる
    ...
    if not os.path.exists(auto_csv_path3):        
        df_E_3 = pd.DataFrame(columns = ['theta','a','za','b','zb','E3','E4','machine_type','status','file_name'])##いじる
...
def get_opt_params_dict(df_cur, init_params_dict,fixed_params_dict):
    ...
    while True:
        E_list=[];xyz_list=[]
        para_list=[]
        for a in [a_init_prev]:#-0.1,a1_init_prev,a1_init_prev+0.1]
            for za in [za_init_prev]:
                    for b in [b_init_prev]:
                        for zb in [zb_init_prev]:
                            a = np.round(a,1);za = np.round(za,1)
                            b = np.round(b,1);zb = np.round(zb,1)
                            df_val_xyz = df_val[(df_val['a']==a)&(df_val['za']==za)&
                                                (df_val['b']==b)&(df_val['zb']==zb)&
                                                (df_val['status']=='Done')]
                            if len(df_val_xyz)==0:
