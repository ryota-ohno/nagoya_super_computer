...
def main_process(args):
    ...
    if not os.path.exists(auto_csv_path):        
        df_E = pd.DataFrame(columns = ['a','b','theta','A1','A2','E','E_p1','E_p2','E_t','machine_type','status','file_name'])##いじる
...
def get_params_dict(auto_dir, num_init,fixed_param_keys,opt_param_keys):
    ...
    for index in df_init_params_inprogress.index:##こちら側はinit_params内のある業に関する探索が終わった際の新しい行での探索を開始するもの ###ここを改良すればよさそう
        ...
        if isDone:
            opt_params_dict={'theta':opt_params_matrix[0][0],'A2':opt_params_matrix[0][1],'a':opt_params_matrix[0][2],'b':opt_params_matrix[0][3]}
            ...
            for i in range(len(opt_params_matrix)):
                opt_params_dict={'theta':opt_params_matrix[i][0],'A2':opt_params_matrix[i][1],'a':opt_params_matrix[i][2],'b':opt_params_matrix[i][3]}
...
def get_opt_params_dict(df_cur, init_params_dict,fixed_params_dict):
    ...
    A2_init_prev = init_params_dict['A2']; A1 = init_params_dict['A1']; theta_init_prev = init_params_dict['theta']
            ...
            for A2 in [A2_init_prev-1,A2_init_prev,A2_init_prev+1]:
                for a in [a_init_prev-0.1,a_init_prev,a_init_prev+0.1]:
                    for b in [b_init_prev-0.1,b_init_prev,b_init_prev+0.1]:
                        theta=np.round(theta,1);A2 = np.round(A2,1)
                        ...
                        if len(df_val_ab)==0:
                            para_list.append([theta,A2,a,b])
                            continue
                        heri_list.append([theta,A2,a,b]);E_list.append(df_val_ab['E'].values[0])
        if len(para_list) != 0:
            return False,para_list
        theta_init,A2_init,a_init,b_init = heri_list[np.argmin(np.array(E_list))]
        if theta_init==theta_init_prev and A2_init==A2_init_prev and a_init==a_init_prev and b_init==b_init_prev:
            return True,[[theta_init,A2_init,a_init,b_init]]
        else:
            theta_init_prev=theta_init;A2_init_prev=A2_init;a_init_prev=a_init;b_init_prev=b_init
