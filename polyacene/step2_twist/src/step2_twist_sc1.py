...
def get_params_dict(auto_dir, num_init,fixed_param_keys,opt_param_keys):
    ...
    for index in df_init_params_inprogress.index:##こちら側はinit_params内のある業に関する探索が終わった際の新しい行での探索を開始するもの ###ここを改良すればよさそう
        ...
        if isDone:
            opt_params_dict={'A2':opt_params_matrix[0][0],'b':opt_params_matrix[0][1]}
            ...
            for i in range(len(opt_params_matrix)):
                opt_params_dict={'A2':opt_params_matrix[i][0],'b':opt_params_matrix[i][1]}
...
def get_opt_params_dict(df_cur, init_params_dict,fixed_params_dict):
    ...
    A2_init_prev = init_params_dict['A2']; b_init_prev = init_params_dict['b']
    A1 = init_params_dict['A1']; a = init_params_dict['a']; theta = init_params_dict['theta']
    ...
    while True:
        ...
        for A2 in [A2_init_prev-1,A2_init_prev,A2_init_prev+1]:
            for b in [b_init_prev-0.1,b_init_prev,b_init_prev+0.1]:
                A2 = np.round(A2,1);b = np.round(b,1)
                df_val_ab = df_val[
                ...
                if len(df_val_ab)==0:
                    para_list.append([A2,b])
                    continue
                heri_list.append([A2,b]);E_list.append(df_val_ab['E'].values[0])
        if len(para_list) != 0:
            return False,para_list
        A2_init,b_init = heri_list[np.argmin(np.array(E_list))]
        if A2_init==A2_init_prev and b_init==b_init_prev:
            return True,[[A2_init,b_init]]
        else:
            A2_init_prev=A2_init;b_init_prev=b_init
