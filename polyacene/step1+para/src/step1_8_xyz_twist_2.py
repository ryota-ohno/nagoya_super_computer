...
def main_process(args):
    ...
    auto_csv_path = os.path.join(auto_dir,'step1.csv')
    if not os.path.exists(auto_csv_path):        
        df_E = pd.DataFrame(columns = ['a','b','theta','A1','A2','Rt','Rp','E','E_t1','E_t2','E_p','machine_type','status','file_name'])##いじる
...
def listen(auto_dir,monomer_name,num_nodes,isTest):##args自体を引数に取るか中身をばらして取るかの違い
    ...
    for idx,row in zip(df_queue.index,df_queue.values):
        ...
        if len(E_list)!=3:
            ...
            len_queue-=1
            Ep=float(E_list[0]);Et1=float(E_list[1]);Et2=float(E_list[2])
            E = 2*Ep+2*(Et1+Et2)
            df_E.loc[idx, ['E_p','E_t1','E_t2','E','status']] = [Ep,Et1,Et2,E,'Done']
        ...
        if len(dict_matrix)!=0:#終わりがまだ見えないなら
            for i in range(len(dict_matrix)):
                ...
                if not(alreadyCalculated):
                    ...
                    df_newline = pd.Series({**params_dict,'E':0.,'E_t1':0.,'E_t2':0.,'E_p':0.,'machine_type':'3','status':'InProgress','file_name':file_name})
