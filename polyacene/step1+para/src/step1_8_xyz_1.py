...
def main_process(args):
    ...
    auto_csv_path = os.path.join(auto_dir,'step1.csv')
    if not os.path.exists(auto_csv_path):        
        df_E = pd.DataFrame(columns = ['Rt','Rp','a','b','theta','E','E_t1','E_t2','E_p','machine_type','status','file_name'])##いじる
...
def get_params_dict(auto_dir, num_init,fixed_param_keys,opt_param_keys):
    ...
    for index in df_init_params_inprogress.index:##こちら側はinit_params内のある業に関する探索が終わった際の新しい行での探索を開始するもの ###ここを改良すればよさそう
        ...
        if isDone:
            opt_params_dict={'Rt':opt_params_matrix[0][0],'Rp':opt_params_matrix[0][1]}
            ...
            for i in range(len(opt_params_matrix)):
                opt_params_dict={'Rt':opt_params_matrix[i][0],'Rp':opt_params_matrix[i][1]}
...
def get_opt_params_dict(df_cur, init_params_dict,fixed_params_dict):
    ...
    Rt_init_prev = init_params_dict['Rt']; Rp_init_prev = init_params_dict['Rp']; theta = init_params_dict['theta']
    ...
    while True:
        ...
        if Rt_init==Rt_init_prev and Rp_init==Rp_init_prev:
            return True,[[Rt_init,Rp_init]]
        else:
            Rt_init_prev=Rt_init;Rp_init_prev=Rp_init
