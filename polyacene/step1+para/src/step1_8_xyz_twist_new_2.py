...
from make_8_xyz_twist_new import exec_gjf##計算した点のxyzfileを出す
...
def main_process(args):
    ...
    auto_csv_path = os.path.join(auto_dir,'step1.csv')
    if not os.path.exists(auto_csv_path):        
        df_E = pd.DataFrame(columns = ['a','b','theta','A1','A2','Rt','Rp','E','E_t1','E_t2','E_p1','E_p2','machine_type','status','file_name'])##いじる
...
def listen(auto_dir,monomer_name,num_nodes,isTest):##args自体を引数に取るか中身をばらして取るかの違い
    ...
    fixed_param_keys = ['theta','Rt','Rp','A1','A2']
    opt_param_keys = ['a','b']
    ...
    for idx,row in zip(df_queue.index,df_queue.values):
        ...
        if len(E_list)!=4:
            ...
            len_queue-=1
            Ep1=float(E_list[0]);Ep2=float(E_list[1]);Et1=float(E_list[2]);Et2=float(E_list[3])
            E = Ep1+Ep2+2*(Et1+Et2)
            df_E.loc[idx, ['E_p1','E_p2','E_t1','E_t2','E','status']] = [Ep1,Ep2,Et1,Et2,E,'Done']
        ...
        if len(dict_matrix)!=0:#終わりがまだ見えないなら
            for i in range(len(dict_matrix)):
                ...
                if not(alreadyCalculated):
                    ...
                    df_newline = pd.Series({**params_dict,'E':0.,'E_t1':0.,'E_t2':0.,'E_p1':0.,'E_p2':0.,'machine_type':'3','status':'InProgress','file_name':file_name})
...
def get_opt_params_dict(df_cur, init_params_dict,fixed_params_dict):
    ...
    Rt = init_params_dict['Rt']; Rp = init_params_dict['Rp'];A1 = init_params_dict['A1']; A2 = init_params_dict['A2']; theta = init_params_dict['theta']
    
    while True:
        E_list=[];heri_list=[]
        para_list=[]
        for a in [a_init_prev-0.05,a_init_prev,a_init_prev+0.05]:
            for b in [b_init_prev-0.05,b_init_prev,b_init_prev+0.05]:
                a = np.round(a,2);b = np.round(b,2)
                df_val_ab = df_val[
                    (df_val['a']==a)&(df_val['b']==b)&(df_val['theta']==theta)&
                    (df_val['Rt']==Rt)&(df_val['Rp']==Rp)&(df_val['A1']==A1)&(df_val['A2']==A2)&
                    (df_val['status']=='Done')
                                 ]
                if len(df_val_ab)==0:
