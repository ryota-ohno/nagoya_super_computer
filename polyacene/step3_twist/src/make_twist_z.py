...
def make_gjf_xyz(auto_dir,monomer_name,params_dict):##R3:t-shaped R4:slipped-parallel
    ...
    dimer_array_ip3 = np.concatenate([monomer_array_c1,monomer_array_ip1])
    dimer_array_ip4 = np.concatenate([monomer_array_c1,monomer_array_ip2])
    ...
    if monomer_name in MONOMER_LIST:# and isInterlayer:
        gij_xyz_lines = ['$ RunGauss\n'] + line_list_dimer_i01 + ['\n\n--Link1--\n'] + line_list_dimer_ip1+ ['\n\n--Link1--\n'] + line_list_dimer_ip2 + ['\n\n--Link1--\n'] + line_list_dimer_it1 + ['\n\n--Link1--\n'] + line_list_dimer_it2 + ['\n\n--Link1--\n'] + line_list_dimer_it3 + ['\n\n--Link1--\n'] + line_list_dimer_it4 + line_list_dimer_i02 + ['\n\n--Link1--\n'] + line_list_dimer_ip3+ ['\n\n--Link1--\n'] + line_list_dimer_ip4 + ['\n\n\n']##2層目9分子
...
def get_file_name_from_dict(monomer_name,paras_dict):
    ...
    for key,val in paras_dict.items():
        if key in ['a','b','cx','cy','cz','theta','phi']:
            val = np.round(val,2)
        elif key in ['A1','A2']:#,'theta']:
            val = np.round(val,2)
