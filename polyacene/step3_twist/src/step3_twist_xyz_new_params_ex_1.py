...
from make_twist_new_params_ex import exec_gjf##計算した点のxyzfileを出す
...
def main_process(args):
    ...
    if not os.path.exists(auto_csv_path):        
        df_E = pd.DataFrame(columns = ['cx','cy','cz','a','b','theta','Rt','A2','E','E_i01','E_ip1','E_ip2','E_i02','E_ip3','E_ip4','E_it1','E_it2','E_it3','E_it4','machine_type','status','file_name'])##いじる
...
def listen(auto_dir,monomer_name,num_nodes,isTest):##args自体を引数に取るか中身をばらして取るかの違い
    ...
    for idx,row in zip(df_queue.index,df_queue.values):
        ...
        if len(E_list)!=10:
            ...
            len_queue-=1
            Ei01=float(E_list[0]);Eip1=float(E_list[1]);Eip2=float(E_list[2]);Ei02=float(E_list[3]);Eip3=float(E_list[4]);Eip4=float(E_list[5]);Eit1=float(E_list[6]);Eit2=float(E_list[7]);Eit3=float(E_list[8]);Eit4=float(E_list[9])##ここも計算する分子数に合わせて調整
            E = (Ei01+Eip1+Eip2+Ei02+Eip3+Eip4)/2+Eit1+Eit2+Eit3+Eit4##隣接20分子　2パターン
            #### TODO
            df_E.loc[idx, ['E_i01','E_ip1','E_ip2','E_i02','E_ip3','E_ip4','E_it1','E_it2','E_it3','E_it4','E','status']] = [Ei01,Eip1,Eip2,Ei02,Eip3,Eip4,Eit1,Eit2,Eit3,Eit4,E,'Done']
        ...
        if len(dict_matrix)!=0:#終わりがまだ見えないなら
            for i in range(len(dict_matrix)):
                ...
                if not(alreadyCalculated):
                    ...
                    df_newline = pd.Series({**params_dict,'E':0.,'E_i01':0.,'E_ip1':0.,'E_ip2':0.,'E_i02':0.,'E_ip3':0.,'E_ip4':0.,'E_it1':0.,'E_it2':0.,'E_it3':0.,'E_it4':0.,'machine_type':'3','status':'InProgress','file_name':file_name})
