...
from make_twist_3 import exec_gjf##計算した点のxyzfileを出す
...
def main_process(args):
    ...
    auto_csv_path = os.path.join(auto_dir,'step3_twist.csv')
    if not os.path.exists(auto_csv_path):        
        df_E = pd.DataFrame(columns = ['cx','cy','cz','a','b','theta','A1','A2','E','E_ip3','machine_type','status','file_name'])##いじる
...
def listen(auto_dir,monomer_name,num_nodes,isTest):##args自体を引数に取るか中身をばらして取るかの違い
    ...
    for idx,row in zip(df_queue.index,df_queue.values):
        ...
        if len(E_list)!=1:
            ...
            len_queue-=1
            Eip3=float(E_list[0])##ここも計算する分子数に合わせて調整
            E = Eip3##隣接20分子　2パターン
            #### TODO
            df_E.loc[idx, ['E_ip3','E','status']] = [Eip3,E,'Done']
        ...
        if len(dict_matrix)!=0:#終わりがまだ見えないなら
            for i in range(len(dict_matrix)):
                ...
                if not(alreadyCalculated):
                    ...
                    df_newline = pd.Series({**params_dict,'E':0.,'E_ip3':0.,'machine_type':'3','status':'InProgress','file_name':file_name})
