...
from make_twist_2 import exec_gjf##計算した点のxyzfileを出す
...
def main_process(args):
    ...
    auto_csv_path = os.path.join(auto_dir,'step3_twist.csv')
    if not os.path.exists(auto_csv_path):        
        df_E = pd.DataFrame(columns = ['cx','cy','cz','a','b','theta','A1','A2','E','E_p1','E_p2','E_t','E_i01','E_ip1','E_ip2','E_it1','E_it2','E_it3','E_it4','machine_type','status','file_name'])##いじる
...
def listen(auto_dir,monomer_name,num_nodes,isTest):##args自体を引数に取るか中身をばらして取るかの違い
    ...
    fixed_param_keys = ['cx','cz','theta','A1','A2']
    opt_param_keys = ['a','b','cy']
    ...
    for idx,row in zip(df_queue.index,df_queue.values):
        ...
        if len(E_list)!=8:
            ...
            len_queue-=1
            Ep1=float(E_list[0]);Ep2=float(E_list[1]);Et=float(E_list[2]);Ei01=float(E_list[3]);Eip1=float(E_list[4]);Eip2=float(E_list[5]);Eit1=float(E_list[6]);Eit2=float(E_list[7]);Eit3=float(E_list[7]);Eit4=float(E_list[6])##ここも計算する分子数に合わせて調整
            E = (Ep1+Ep2)*2+Et*4+(Ei01+Eip1+Eip2+Eit1+Eit2+Eit3+Eit4)*2##隣接22分子　2パターン
            #### TODO
            df_E.loc[idx, ['E_p1','E_p2','E_t','E_i01','E_ip1','E_ip2','E_it1','E_it2','E_it3','E_it4','E','status']] = [Ep1,Ep2,Et,Ei01,Eip1,Eip2,Eit1,Eit2,Eit3,Eit4,E,'Done']
        ...
        if len(dict_matrix)!=0:#終わりがまだ見えないなら
            for i in range(len(dict_matrix)):
                ...
                if not(alreadyCalculated):
                    ...
                    df_newline = pd.Series({**params_dict,'E':0.,'E_p1':0.,'E_p2':0.,'E_t':0.,'E_i01':0.,'E_ip1':0.,'E_ip2':0.,'E_it1':0.,'E_it2':0.,'E_it3':0.,'E_it4':0.,'machine_type':'3','status':'InProgress','file_name':file_name})
...
def get_params_dict(auto_dir, num_init,fixed_param_keys,opt_param_keys):
    ...
    for index in df_init_params_inprogress.index:##こちら側はinit_params内のある業に関する探索が終わった際の新しい行での探索を開始するもの ###ここを改良すればよさそう
        ...
        if isDone:
            opt_params_dict={'a':opt_params_matrix[0][0],'b':opt_params_matrix[0][1],'cy':opt_params_matrix[0][2]}
            ...
            for i in range(len(opt_params_matrix)):
                opt_params_dict={'a':opt_params_matrix[i][0],'b':opt_params_matrix[i][1],'cy':opt_params_matrix[i][2]}
...
def get_opt_params_dict(df_cur, init_params_dict,fixed_params_dict):
    ...
    cx = init_params_dict['cx']; cy_init_prev = init_params_dict['cy']; cz = init_params_dict['cz']
    ...
    while True:
        E_list=[];heri_list=[]
        para_list=[]
        for cy in [cy_init_prev-1,cy_init_prev,cy_init_prev+1]:
            for a in [a_init_prev-0.1,a_init_prev,a_init_prev+0.1]:
                for b in [b_init_prev-0.1,b_init_prev,b_init_prev+0.1]:
                    a = np.round(a,1);b = np.round(b,1);cy = np.round(cy,1)
                    df_val_ab = df_val[
                    (df_val['cx']==cx)&(df_val['cy']==cy)&(df_val['cz']==cz)&(df_val['A1']==A1)&(df_val['A2']==A2)&(df_val['theta']==theta)&(df_val['a']==a)&(df_val['b']==b)&
                    (df_val['status']=='Done')
                                     ]
                    if len(df_val_ab)==0:
                        para_list.append([a,b,cy])
                        continue
                    heri_list.append([a,b,cy]);E_list.append(df_val_ab['E'].values[0])
        if len(para_list) != 0:
            return False,para_list
        a_init,b_init,cy_init = heri_list[np.argmin(np.array(E_list))]
        if a_init==a_init_prev and b_init==b_init_prev and cy_init==cy_init_prev:
            return True,[[a_init,b_init,cy_init]]
        else:
            a_init_prev=a_init;b_init_prev=b_init;cy_init_prev=cy_init
