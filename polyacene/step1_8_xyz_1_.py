...
def init_process(args):
    ...
    def get_init_para_csv(auto_dir,monomer_name):
        ...
        init_para_list = []
        A1 = 0; A2 = 0
        theta_list = [15,25,35,45]#list(range(0,95,5)) ##+ [22.5,23.0,23.5,24.0,24.5] + [25.5,26.0,26.5,27.0,27.5]##全体を掃く＋細かいところ
...
def main_process(args):
    ...
    auto_csv_path = os.path.join(auto_dir,'step1.csv')
    if not os.path.exists(auto_csv_path):        
        df_E = pd.DataFrame(columns = ['a','b','theta','E','E_t1','E_t2','E_p1','Ga','Gb','Gc','machine_type','status','file_name'])##いじる
...
def listen(auto_dir,monomer_name,num_nodes,isTest):##args自体を引数に取るか中身をばらして取るかの違い
    ...
    fixed_param_keys = ['theta','Ga','Gb','Gc']
    opt_param_keys = ['a','b']
    ...
    for idx,row in zip(df_queue.index,df_queue.values):
        ...
        if len(E_list)!=3:
            ...
            len_queue-=1
            Et1=float(E_list[0]);Et2=float(E_list[1]);Ep1=float(E_list[2])
            E = 2*(Et1+Et2)+Ep1
        ...
        if len(dict_matrix)!=0:#終わりがまだ見えないなら
            for i in range(len(dict_matrix)):
                ...
                if not(alreadyCalculated):
                    ...
                    df_newline = pd.Series({**params_dict,'E':0.,'E_t1':0.,'E_t2':0.,'E_p1':0.,'machine_type':'3','status':'InProgress','file_name':file_name})
