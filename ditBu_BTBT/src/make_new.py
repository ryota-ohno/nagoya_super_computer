...
def get_monomer_xyzR(monomer_name,Ta,Tb,Tc,A2,A3,phi):
    ...
    df_mono=pd.read_csv('~/Working/nagoya_super_computer/ditBu_BTBT/monomer/{}.csv'.format(monomer_name))
    ...
    xyz_array[C3_index:C1_index] = np.matmul((xyz_array[C3_index:C1_index]-C2),Rod(n2,phi).T) + C2
...
def make_gaussview_xyz(auto_dir,monomer_name,params_dict,isInterlayer=False):
    ...
    if a_>b_:
        monomer_array_p1 = get_monomer_xyzR(monomer_name,0,b_,2*z,A2,A3, phi1)
        monomer_array_p2 = get_monomer_xyzR(monomer_name,0,-b_,-2*z,A2,A3, phi1)
    else:
    ...
    monomer_array_t1 = get_monomer_xyzR(monomer_name,a_/2,b_/2,z,A2,-A3, phi2)
    monomer_array_t2 = get_monomer_xyzR(monomer_name,a_/2,-b_/2,z,A2,-A3, phi2)
    monomer_array_t3 = get_monomer_xyzR(monomer_name,-a_/2,-b_/2,z,A2,-A3, phi2)
    monomer_array_t4 = get_monomer_xyzR(monomer_name,-a_/2,b_/2,z,A2,-A3, phi2)
...
def get_file_name_from_dict(monomer_name,paras_dict):
    ...
    for key,val in paras_dict.items():
        if key in ['a','b','z','cx','cy','cz','theta']:
            val = np.round(val,2)
        elif key in ['A2','phi']:
            val = int(val)
