...
MONOMER_LIST = ["BTBT","benzene","naphthalene","anthracene","tetracene","pentacene","hexacene","demo","PTA","TTB"]
############################汎用関数###########################
def get_monomer_xyzR(monomer_name,Ta,Tb,Tc,A1,A2,A3,phi=0.0,isFF=False):
    T_vec = np.array([Ta,Tb,Tc])
    df_mono=pd.read_csv('~/Working/nagoya_super_computer/TTB/monomer/{}.csv'.format(monomer_name))
...
def get_monomer_xyzR_(monomer_name,Ta,Tb,Tc,A1,A2,A3,phi=0.0,isFF=False):
    ...
    xyz_array = atoms_array_xyzR[:,:3]
    xyz_array = np.matmul(xyz_array,Rod(ex,180).T)
    xyz_array = np.matmul(xyz_array,Rod(ez,A3).T)
    xyz_array = np.matmul(xyz_array,Rod(-ex,A2).T)
    xyz_array = np.matmul(xyz_array,Rod(ey,A1).T)
    xyz_array = xyz_array + T_vec
...
def make_gjf_xyz(auto_dir,monomer_name,params_dict,isInterlayer):
    ...
    monomer_array_t1 = get_monomer_xyzR_(monomer_name,a_/2+0.01,b_/2+0.01,Rt+0.01,-A1,A2,-A3, phi2)
    monomer_array_t2 = get_monomer_xyzR_(monomer_name,a_/2+0.01,-b_/2+0.01,Rt-Rp+0.01,-A1,A2,-A3, phi2)
    monomer_array_t3 = get_monomer_xyzR_(monomer_name,-a_/2+0.01,-b_/2+0.01,-Rt+0.01,-A1,A2,-A3, phi2)
    monomer_array_t4 = get_monomer_xyzR_(monomer_name,-a_/2+0.01,b_/2+0.01,-Rt+Rp+0.01,-A1,A2,-A3, phi2)
