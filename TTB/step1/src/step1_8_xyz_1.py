...
def init_process(args):
    ...
    def get_init_para_csv(auto_dir,monomer_name):
        ...
        init_para_list = []
        A1 = 0; A2 = 0
        theta_list = [5,10,15,20,25,30,35,40,45,50,55,60,65,70,75,80,85]#list(range(0,95,5)) ##+ [22.5,23.0,23.5,24.0,24.5] + [25.5,26.0,26.5,27.0,27.5]##全体を掃く＋細かいところ
        for theta in tqdm(theta_list):
            ...
            for theta_ab in range(0,91):
                ...
                R_clps_=vdw_R(A1,A2,theta,360-theta_ab,'t',monomer_name)
    ...
    if not os.path.exists(auto_csv_path):        
        df_E_init = pd.DataFrame(columns = ['a','b','theta','E','E_p1','E_p2','E_t1','E_t3','machine_type','status','file_name'])##隣接8分子
    else:
...
def main_process(args):
    ...
    auto_csv_path = os.path.join(auto_dir,'step1.csv')
    if not os.path.exists(auto_csv_path):        
        df_E = pd.DataFrame(columns = ['a','b','theta','Rt','Rp','E','E_p1','E_p2','E_t1','E_t3','machine_type','status','file_name'])##いじる
...
def listen(auto_dir,monomer_name,num_nodes,isTest):##args自体を引数に取るか中身をばらして取るかの違い
    ...
    for idx,row in zip(df_queue.index,df_queue.values):
        ...
        if len(E_list)!=4:
            ...
            len_queue-=1
            Et1=float(E_list[0]);Et3=float(E_list[1]);Ep1=float(E_list[2]);Ep2=float(E_list[3])
            E = 2*(Et1+Et3+Ep1+Ep2)
