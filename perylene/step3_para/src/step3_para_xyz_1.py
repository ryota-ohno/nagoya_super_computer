...
def main_process(args):
    ...
    auto_csv_path = os.path.join(auto_dir,'step3_para.csv')
    if not os.path.exists(auto_csv_path):        
        df_E = pd.DataFrame(columns = ['cx','cy','cz','a','b','theta','Rt','Rp','E','E_i01','E_ip1','E_ip2','E_ip3','E_ip4','E_it1','E_it2','E_it3','E_it4','machine_type','status','file_name'])##いじる
...
def listen(auto_dir,monomer_name,num_nodes,isTest):##args自体を引数に取るか中身をばらして取るかの違い
    ...
    fixed_param_keys = ['theta','a','b','Rt','Rp']
    ...
    for idx,row in zip(df_queue.index,df_queue.values):
        ...
        if len(E_list)!=7:
            ...
            len_queue-=1
            Ei01=float(E_list[0]);Eip1=float(E_list[1]);Eip2=float(E_list[2]);Eip3=float(E_list[3]);Eip4=float(E_list[4]);Eit1=float(E_list[5]);Eit2=Eit1;Eit3=float(E_list[6]);Eit4=Eit3##ここも計算する分子数に合わせて調整
            E = Ei01 + Eip1 + Eip2 + Eip3 + Eip4 + Eit1 + Eit2 + Eit3 + Eit4##隣接20分子　2パターン
            #### TODO
            df_E.loc[idx, ['E_i01','E_ip1','E_ip2','E_ip3','E_ip4','E_it1','E_it2','E_it3','E_it4','E','status']] = [Ei01,Eip1,Eip2,Eip3,Eip4,Eit1,Eit2,Eit3,Eit4,E,'Done']
        ...
        if len(dict_matrix)!=0:#終わりがまだ見えないなら
            for i in range(len(dict_matrix)):
                ...
                if not(alreadyCalculated):
                    ...
                    df_newline = pd.Series({**params_dict,'E':0.,'E_i01':0.,'E_ip1':0.,'E_ip2':0.,'E_ip3':0.,'E_ip4':0.,'E_it1':0.,'E_it2':0.,'E_it3':0.,'E_it4':0.,'machine_type':'3','status':'InProgress','file_name':file_name})
...
def get_opt_params_dict(df_cur, init_params_dict,fixed_params_dict):
    ...
    a = init_params_dict['a'];b = init_params_dict['b'];theta = init_params_dict['theta'];Rt = init_params_dict['Rt'];Rp = init_params_dict['Rp']
        ...
        for cx in [cx_init_prev-0.1,cx_init_prev,cx_init_prev+0.1]:
            for cy in [cy_init_prev]:
                for cz in [cz_init_prev-0.1,cz_init_prev,cz_init_prev+0.1]:
                    ...
                    df_val_ab = df_val[
                        (df_val['cx']==cx)&(df_val['cy']==cy)&(df_val['cz']==cz)&(df_val['Rt']==Rt)&(df_val['Rp']==Rp)&(df_val['a']==a)&(df_val['b']==b)&(df_val['theta']==theta)&
                        (df_val['status']=='Done')
                                     ]
                    if len(df_val_ab)==0:
