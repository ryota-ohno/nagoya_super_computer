...
def main_process(args):
    ...
    shutil.copy(f'/data/group1/z40145w/Working/nagoya_super_computer/amber_sc_opt/{args.monomer_name}/monomer/BTBT_mono.frcmod',amber_path)
    
    auto_csv_path = os.path.join(auto_dir,'step1.csv')
    if not os.path.exists(auto_csv_path): 
        header = ['theta','A2','a','b','z','E','E1','E2','E3','status']
    ...
    if not os.path.exists(auto_csv_path1):
        header = ['theta','A2','a','E1','status','file_name']
    ...
    if not os.path.exists(auto_csv_path2):
        header = ['theta','A2','b','z','E2','status','file_name']
    ...
    if not os.path.exists(auto_csv_path3):        
        header = ['theta','A2','a','b','z','E3','status','file_name']
...
def listen(auto_dir,monomer_name,num_nodes,isTest):##args自体を引数に取るか中身をばらして取るかの違い
    ...
    for row in df:
        ...
        if not s1 or not s2 or not s3:
            continue
        E1 = float(s1[0]['E1']);E2 = float(s2[0]['E2']);E3 = float(s3[0]['E3'])
        E = 2 * E1 + 2 * E2 + 4 * E3
        row['E1'] = f"{E1:.4f}";row['E2'] = f"{E2:.4f}";row['E3'] = f"{E3:.4f}"
