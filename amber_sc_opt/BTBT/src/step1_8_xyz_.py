...
def main_process(args):
    ...
    if not os.path.exists(auto_csv_path): 
        header = ['theta','a','b','E','E1','E2','E3','status']
    ...
    if not os.path.exists(auto_csv_path1):
        header = ['theta','a','E1','status','file_name']
    ...
    if not os.path.exists(auto_csv_path2):
        header = ['theta','b','E2','status','file_name']
    ...
    if not os.path.exists(auto_csv_path3):        
        header = ['theta','a','b','E3','status','file_name']
...
def get_params_dict(auto_dir, num_nodes):
    ...
    if len(dictlist_init_params_notyet) != 0:
        if len(dictlist_init_params_inprogress) < num_nodes:
            ...
            return [params_dict]
    with open(os.path.join(auto_dir, 'debug1.txt'),'w')as f:
        f.write('debug1')
    ...
    with open(os.path.join(auto_dir, 'debug2.txt'),'w')as f:
        f.write(f'debug2 {len(dict_matrix)}')
    return dict_matrix
