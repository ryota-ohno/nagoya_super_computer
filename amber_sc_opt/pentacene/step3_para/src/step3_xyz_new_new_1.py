...
import time
from make_xyz_new_new import exec_gjf,exec_mono_gjf##計算した点のxyzfileを出す
...
def init_process(args):
    ...
    fixed_param_keys = ['theta','a','b','z1','z2']
    fixed_params_dict = get_values_from_dictlist(dictlist_init_params, 0, fixed_param_keys)
    file_name = exec_mono_gjf(auto_dir, args.monomer_name, {**fixed_params_dict},isTest=False)
    while True:
        mono_file_1=os.path.join(auto_dir, 'amber', 'layer_3.out')
        if os.path.exists(mono_file_1):
            E_mono3=get_E(mono_file_1)
            if len(E_mono3)>0:
                break
...
def listen(auto_dir,monomer_name,num_nodes,isTest):##args自体を引数に取るか中身をばらして取るかの違い
    ...
    mono_file=f'/data/group1/z40145w/Working/nagoya_super_computer/amber_sc_opt/{monomer_name}/monomer/{monomer_name}_mono_.out'
    E_mono=get_E(mono_file)[0]
    mono_file_1=os.path.join(auto_dir, 'amber', 'layer_1.out')
    E_mono1=get_E(mono_file_1)[0]
    mono_file_2=os.path.join(auto_dir, 'amber', 'layer_2.out')
    E_mono2=get_E(mono_file_2)[0]
    mono_file_3=os.path.join(auto_dir, 'amber', 'layer_3.out')
    E_mono3=get_E(mono_file_3)[0]
    ...
    for idx, row in enumerate(rows_1):
        ...
        log_filepath1 = os.path.join(auto_dir, 'amber', file_name1+'_1.out')
        if not os.path.exists(log_filepath1):
            continue
        log_filepath2 = os.path.join(auto_dir, 'amber', file_name1+'_2.out')
        if not os.path.exists(log_filepath2):
            continue
        log_filepath3 = os.path.join(auto_dir, 'amber', file_name1+'_3.out')
        if not os.path.exists(log_filepath3):
            continue
        E_list1=get_E(log_filepath1);E_list2=get_E(log_filepath2);E_list3=get_E(log_filepath3)
        if len(E_list1) != 1:
            continue
        if len(E_list2) != 1:
            continue
        if len(E_list3) != 1:
            continue
        E1=round(float(E_list1[0]) - E_mono - E_mono1 , 4)
        E2=round(float(E_list2[0]) - E_mono - E_mono2 , 4)
        E3=round(float(E_list3[0]) - E_mono - E_mono3 , 4)
        E_list=[E1,E2,E3]
        E_columns=['E1','E2','E3']
        ...
        for i in range(len(dict_matrix)):
            ...
            if not(alreadyCalculated):
                ...
                if len(dict_list_E1_f) == 0:
                    ...
                    new_dict_1 = {**params_dict1,'E':'0.0','E1':'0.0','E2':'0.0','E3':'0.0', 'status': 'InProgress','file_name':file_name}
