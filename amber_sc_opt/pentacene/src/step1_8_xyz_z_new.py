...
def main_process(args):
    ...
    if not os.path.exists(auto_csv_path): 
        header = ['theta','a','b','z1','z2','za','zb','E','E1','E2','E3','E4','status']
    ...
    if not os.path.exists(auto_csv_path1):
        header = ['theta','a','za','E1','status','file_name']
    ...
    if not os.path.exists(auto_csv_path2):
        header = ['theta','b','zb','E2','status','file_name']
    ...
    if not os.path.exists(auto_csv_path3):        
        header = ['theta','a','b','z1','E3','status','file_name']
    ...
    if not os.path.exists(auto_csv_path4):        
        header = ['theta','a','b','z2','E4','status','file_name']
...
def listen(auto_dir,monomer_name,num_nodes,isTest):##args自体を引数に取るか中身をばらして取るかの違い
    ...
    for idx, row in enumerate(rows_1):
        if row['status'] != 'InProgress':
            continue
        params_dict1_ = {key: row[key] for key in fixed_param_keys + opt_param_keys_1 + ['za','file_name']}
    ...
    for idx, row in enumerate(rows_2):
        if row['status'] != 'InProgress':
            continue
        params_dict2_ = {key: row[key] for key in fixed_param_keys + opt_param_keys_2 + ['zb','file_name']}
    ...
    for idx, row in enumerate(rows_3):
        if row['status'] != 'InProgress':
            continue
        params_dict3_ = {key: row[key] for key in fixed_param_keys + opt_param_keys_1 + opt_param_keys_2 + ['z1','file_name']}
    ...
    for idx, row in enumerate(rows_4):
        if row['status'] != 'InProgress':
            continue
        params_dict4_ = {key: row[key] for key in fixed_param_keys + opt_param_keys_1 + opt_param_keys_2 + ['z2','file_name']}
    ...
    for row in df:
        if row['status'] != 'InProgress':
            continue
        params_dict1_ = {k: row[k] for k in fixed_param_keys + opt_param_keys_1 + ['za']}
        params_dict2_ = {k: row[k] for k in fixed_param_keys + opt_param_keys_2 + ['zb']}
        params_dict3_ = {k: row[k] for k in fixed_param_keys + opt_param_keys_1 + opt_param_keys_2 + ['z1']}
        params_dict4_ = {k: row[k] for k in fixed_param_keys + opt_param_keys_1 + opt_param_keys_2 + ['z2']}
    ...
    if len(dict_matrix)!=0:#終わりがまだ見えないなら
        for i in range(len(dict_matrix)):
            ...
            params_dict1 = {k: v for k, v in params_dict.items() if (k in fixed_param_keys) or (k in opt_param_keys_1) or (k in ['za'])}
            params_dict2 = {k: v for k, v in params_dict.items() if (k in fixed_param_keys) or (k in opt_param_keys_2) or (k in ['zb'])}
            params_dict3 = {k: v for k, v in params_dict.items() if (k in fixed_param_keys) or (k in opt_param_keys_1) or (k in opt_param_keys_2) or (k in ['z1'])}
            params_dict4 = {k: v for k, v in params_dict.items() if (k in fixed_param_keys) or (k in opt_param_keys_1) or (k in opt_param_keys_2) or (k in ['z2'])}
...
def get_params_dict(auto_dir, num_nodes):
    ...
    fixed_param_keys = ['theta','z1','z2','za','zb'];opt_param_keys_1 = ['a'];opt_param_keys_2 = ['b']
