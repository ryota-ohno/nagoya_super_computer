...
def main_process(args):
    ...
    if not os.path.exists(auto_csv_path):        
        df_E = pd.DataFrame(columns = ['a','b','theta','A1','A2','phi_r','E','E_p1','E_p2','E_t1','E_t3','machine_type','status','file_name'])##いじる
...
def listen(auto_dir,monomer_name,num_nodes,isTest):##args自体を引数に取るか中身をばらして取るかの違い
    ...
    for idx,row in zip(df_queue.index,df_queue.values):
        ...
        if len(E_list)!=4:
            ...
            E = 2*(Ep1+Ep2+Et1+Et3)
            df_E.loc[idx, ['E_t1','E_t3','E_p1','E_p2','E','status']] = [Et1,Et3,Ep1,Ep2,E,'Done']
        ...
        if len(dict_matrix)!=0:#終わりがまだ見えないなら
            for i in range(len(dict_matrix)):
                ...
                if not(alreadyCalculated):
                    ...
                    df_newline = pd.Series({**params_dict,'E':0.,'E_p1':0.,'E_p2':0.,'E_t1':0.,'E_t3':0.,'machine_type':'3','status':'InProgress','file_name':file_name})
...
def get_params_dict(auto_dir, num_init,fixed_param_keys,opt_param_keys):
    ...
    for index in df_init_params_inprogress.index:##こちら側はinit_params内のある業に関する探索が終わった際の新しい行での探索を開始するもの ###ここを改良すればよさそう
        ...
        if isDone:
            opt_params_dict={'a':opt_params_matrix[0][0],'b':opt_params_matrix[0][1],'phi_r':opt_params_matrix[0][2]}
            ...
            for i in range(len(opt_params_matrix)):
                opt_params_dict={'a':opt_params_matrix[i][0],'b':opt_params_matrix[i][1],'phi_r':opt_params_matrix[i][2]}
...
def get_opt_params_dict(df_cur, init_params_dict,fixed_params_dict):
    ...
    a_init_prev = init_params_dict['a']; b_init_prev = init_params_dict['b']; phi_r_init_prev = init_params_dict['phi_r']
        ...
        for a in [a_init_prev-0.1,a_init_prev,a_init_prev+0.1]:
            for b in [b_init_prev-0.1,b_init_prev,b_init_prev+0.1]:
                for phi_r in [phi_r_init_prev-1,phi_r_init_prev,phi_r_init_prev+1]:
                    ...
                    if len(df_val_ab)==0:
                        para_list.append([a,b,phi_r])
                        continue
                    heri_list.append([a,b,phi_r]);E_list.append(df_val_ab['E'].values[0])
        if len(para_list) != 0:
            return False,para_list
        a_init,b_init,phi_r_init = heri_list[np.argmin(np.array(E_list))]
        if a_init==a_init_prev and b_init==b_init_prev and phi_r_init==phi_r_init_prev:
            return True,[[a_init,b_init,phi_r_init]]
        else:
            a_init_prev=a_init;b_init_prev=b_init;phi_r_init_prev=phi_r_init
