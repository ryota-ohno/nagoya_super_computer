...
from make_new_1 import exec_gjf##計算した点のxyzfileを出す
...
def listen(auto_dir,monomer_name,num_nodes,isTest):##args自体を引数に取るか中身をばらして取るかの違い
    ...
    opt_param_keys = ['a','b','phi_r','phi_b']
...
def get_params_dict(auto_dir, num_init,fixed_param_keys,opt_param_keys):
    ...
    for index in df_init_params_inprogress.index:##こちら側はinit_params内のある業に関する探索が終わった際の新しい行での探索を開始するもの ###ここを改良すればよさそう
        ...
        if isDone:
            opt_params_dict={'a':opt_params_matrix[0][0],'b':opt_params_matrix[0][1],'phi_r':opt_params_matrix[0][2],'phi_b':opt_params_matrix[0][3]}
            ...
            for i in range(len(opt_params_matrix)):
                opt_params_dict={'a':opt_params_matrix[i][0],'b':opt_params_matrix[i][1],'phi_r':opt_params_matrix[i][2],'phi_b':opt_params_matrix[i][3]}
...
def get_opt_params_dict(df_cur, init_params_dict,fixed_params_dict):
    ...
    a_init_prev = init_params_dict['a']; b_init_prev = init_params_dict['b']; phi_r_init_prev = init_params_dict['phi_r']; phi_b_init_prev = init_params_dict['phi_b']
        ...
        for a in [a_init_prev]:
            for b in [b_init_prev]:
                for phi_r in [phi_r_init_prev-1,phi_r_init_prev,phi_r_init_prev+1]:
                    for phi_b in [phi_b_init_prev-2,phi_b_init_prev,phi_b_init_prev+2]:
