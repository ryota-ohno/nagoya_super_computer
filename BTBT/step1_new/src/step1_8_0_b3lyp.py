...
import pandas as pd
import time
from make_8_b3lyp import exec_gjf##計算した点のxyzfileを出す
...
def listen(auto_dir,monomer_name,num_nodes,max_nodes,isTest):##args自体を引数に取るか中身をばらして取るかの違い
    ...
    for idx,row in df_prg.iterrows():
        ...
        s1=filter_df(df_E_1, params_dict1_);s2=filter_df(df_E_2, params_dict2_);s3=filter_df(df_E_3, params_dict3_)
        s1=s1[s1['status']=='Done'];s2=s2[s2['status']=='Done'];s3=s3[s3['status']=='Done']
        if (len(s1) == 0) or (len(s2) == 0) or (len(s3) == 0):
            continue
        else:
            E1=float(s1['E1'].values[0]);E2=float(s2['E2'].values[0]);E3=float(s3['E3'].values[0]);E4=E3
            E=E1+E2+E3+E4
