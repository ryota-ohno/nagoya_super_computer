...
def listen(auto_dir,monomer_name,num_nodes,max_nodes,isTest):##args自体を引数に取るか中身をばらして取るかの違い
    fixed_param_keys = ['theta','A2'];opt_param_keys = ['a','b','z']
    
    auto_csv = os.path.join(auto_dir,'step1.csv');df_E = pd.read_csv(auto_csv)
    df_prg = df_E.loc[df_E['status']=='InProgress',fixed_param_keys+opt_param_keys+['machine_type','file_name']]
    len_prg=len(df_prg)
    for idx, row in df_prg.iterrows():
        params_dict = row[fixed_param_keys + opt_param_keys + ['file_name']].to_dict()
        file_name=params_dict['file_name']##辞書をつくってそこにopt_1とopt_2でファイル名作成
        ...
        if len(E_list)!=3 :##get Eの長さは計算した分子の数
            ...
            len_prg-=1
            E1=float(E_list[0]);E2=float(E_list[1]);E3=float(E_list[2])##8分子に向けてep1,ep2作成　ep1:b ep2:a
            E=2*E1+2*E2+4*E3
            df_E.loc[idx, ['E1','E2','E3','E','status']] = [E1,E2,E3,E,'Done']
            df_E.to_csv(auto_csv,index=False)
            #time.sleep(1)
            break#2つ同時に計算終わったりしたらまずいので一個で切る
    
    df_qw = df_E[df_E['status'] == 'qw']
    len_queue = len_prg;len_qw = len(df_qw)
    margin = max_nodes - len_queue;machine_type=3

    if len_qw > 0 and margin > 0:# 進行中ジョブのマシンタイプをカウント
    ...
    if len(dict_matrix)!=0:#終わりがまだ見えないなら
        for i in range(len(dict_matrix)):
            ...
            if not(alreadyCalculated):##ここで各点について1~3を実行しつつ余ったものをqwにぶち込む
                auto_csv = os.path.join(auto_dir,'step1.csv');df_E = pd.read_csv(auto_csv)
                df_sub = filter_df(df_E, params_dict)
                if len(df_sub) == 0:#(len(df_done_1_)>=1) or(len(df_inpr_1_)>=1) or (len(df_qw_1_)>=1):
                    #print('debug1')
                    isAvailable = len_queue < max_nodes
                    if isAvailable:
                        file_name = exec_gjf(auto_dir, monomer_name, {**params_dict}, machine_type,isTest=isTest);len_queue +=1
                        df_newline = pd.Series({**params_dict,'E1':0.,'E2':0.,'E3':0.,'E':0.,'machine_type':machine_type,'status':'InProgress','file_name':file_name})
                        df_E_new=pd.concat([df_E,df_newline.to_frame().T],axis=0,ignore_index=True);df_E_new.to_csv(auto_csv,index=False)
                        #time.sleep(1)
                    else:
                        file_name = exec_gjf(auto_dir, monomer_name, {**params_dict}, machine_type, isTest=True)
                        df_newline = pd.Series({**params_dict,'E1':0.,'E2':0.,'E3':0.,'E':0.,'machine_type':3,'status':'qw','file_name':file_name})
                        df_E_new=pd.concat([df_E,df_newline.to_frame().T],axis=0,ignore_index=True);df_E_new.to_csv(auto_csv,index=False)
                        #time.sleep(1)
