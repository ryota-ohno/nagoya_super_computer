...
def make_xyzfile(monomer_name,params_dict):
    ...
    monomer_array_p1 = get_monomer_xyza(monomer_name,0,b,2*z,A2,theta1)##1,2がb方向
    monomer_array_p2 = get_monomer_xyza(monomer_name,0,-b,-2*z,A2,theta1)##1,2がb方向
    monomer_array_p3 = get_monomer_xyza(monomer_name,a,0,0,A2,theta1)##3,4がa方向
    monomer_array_p4 = get_monomer_xyza(monomer_name,-a,0,0,A2,theta1)##3,4がa方向
    monomer_array_t1 = get_monomer_xyza(monomer_name,a/2,b/2,z,A2,-theta1)
    monomer_array_t2 = get_monomer_xyza(monomer_name,a/2,-b/2,-z,A2,-theta1)
    monomer_array_t3 = get_monomer_xyza(monomer_name,-a/2,-b/2,-z,A2,-theta1)
    monomer_array_t4 = get_monomer_xyza(monomer_name,-a/2,b/2,z,A2,-theta1)
    xyz_list=['400 \n','polyacene9 \n']##4分子のxyzファイルを作成
...
def make_xyz(monomer_name,params_dict):
    ...
    for key,val in params_dict.items():
        if key in ['a','b','z','cx','cy','cz']:
            val = np.round(val,2)
        elif key in ['A1','A2','theta1','theta2']:#]:
            val = int(val)
...
def make_gjf_xyz(auto_dir,monomer_name,params_dict,isInterlayer):
    ...
    monomer_array_c_ = get_monomer_xyza(monomer_name,cx,cy,cz,0,-theta2)
    monomer_array_i_ = get_monomer_xyza(monomer_name,0,0,0,0,-theta1)
    monomer_array_p1_ = get_monomer_xyza(monomer_name,0,b,2*z,A2,-theta1)##1,2がb方向
    monomer_array_p2_ = get_monomer_xyza(monomer_name,0,-b,-2*z,A2,-theta1)##1,2がb方向
    monomer_array_p3_ = get_monomer_xyza(monomer_name,a,0,0,A2,-theta1)##3,4がa方向
    monomer_array_p4_ = get_monomer_xyza(monomer_name,-a,0,0,A2,-theta1)##3,4がa方向
    
    monomer_array_t1_ = get_monomer_xyza(monomer_name,a/2,b/2,z,A2,theta1)
    monomer_array_t2_ = get_monomer_xyza(monomer_name,a/2,-b/2,-z,A2,theta1)
    monomer_array_t3_ = get_monomer_xyza(monomer_name,-a/2,-b/2,-z,A2,theta1)
    monomer_array_t4_ = get_monomer_xyza(monomer_name,-a/2,b/2,z,A2,theta1)
    ...
    file_description = '{}_theta1={}_theta2={}'.format(monomer_name,int(theta1),int(theta2))
