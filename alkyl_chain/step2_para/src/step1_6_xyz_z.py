...
def init_process(args):
    ...
    df_ = pd.read_csv(os.path.join(auto_dir,'step2_init_params.csv'))
    a=df_['a'].values.tolist()[0];b=df_['b'].values.tolist()[0];theta=df_['theta'].values.tolist()[0]
    para_list=[]
    for i in range(81):
        z=np.round(-4.0+i*0.1,1)
        para_list.append([a,b,theta,z,'NotYet'])
    df_init=pd.DataFrame(para_list,columns=['a','b','theta','z','status'])
    df_init.to_csv(init_csv_path,index=False)
...
def main_process(args):
    ...
    if not os.path.exists(auto_csv_path):        
        df_E = pd.DataFrame(columns = ['a','b','theta','z','E','E_p1','E_p2','E_t1','E_t4','status','file_name'])##いじる
    ...
    if isAvailable:
        ...
        if len(params_dict)!=0:#終わりがまだ見えないなら
            ...
            if not(alreadyCalculated):
                file_name = exec_gjf(auto_dir, monomer_name, {**params_dict,'cx':0,'cy':0,'cz':0,'A1':0.,'A2':0.},isInterlayer=False,isTest=isTest)##計算を実行並びにxyzファイルの出力
                df_newline = pd.Series({**params_dict,'E':0.,'E_p1':0.,'E_p2':0.,'E_t1':0.,'E_t4':0.,'status':'InProgress','file_name':file_name})
